...
if ( ! function_exists( 'travel_tour_setup' ) ) :
	...
	function travel_tour_setup() {
		...
		// Define Image Sizes:
		add_image_size( 'travel-tour-slider', 256, 350, true );
...
function travel_tour_scripts() {
	...
	if ( is_rtl() ) {
		...
		wp_enqueue_script( 'travel-tour-js-rtl', get_template_directory_uri() . '/js/bootstrap.rtl.js', array('jquery'), '1.0.0', true );
	}
	...
	wp_enqueue_script( 'travel-tour-scripts', get_template_directory_uri() . '/js/script.js', array('jquery'), '1.0.0', true );
...
if ( ! class_exists( 'Travel_Tour_Customize' ) ) {
	require get_template_directory() . '/trt-customize-pro/travel-tour/class-customize.php';
}
...
add_filter( 'post_class', 'travel_tour_sticky_classes', 10, 3 ); 
